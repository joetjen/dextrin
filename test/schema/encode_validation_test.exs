defmodule Dextrin.Schema.EncodeValidationTest do
  @moduledoc """
  Encode-time schema validation (DESIGN.md §10/§12): `Dextrin.encode/2`
  ...
  test "a Dextrin.Struct satisfying its schema encodes to named struct-literal wire syntax", %{registry: registry} do
    money = Dextrin.Struct.keyed("Money", [{"amount", Decimal.new("19.99")}, {"currency", Dextrin.Keyword.new("usd")}])
    assert {:ok, "%Money{amount: 19.99M, currency: :usd}"} = Dextrin.encode(money, registry: registry, schema: "Money")
  end
  ...
  test "a missing required field is rejected before encoding", %{registry: registry} do
    ...
    assert {:error, %Dextrin.Error{message: message}} = Dextrin.encode(money, registry: registry, schema: "Money")
    assert message =~ "amount"
  end

  test "an enum violation is rejected before encoding", %{registry: registry} do
    money = Dextrin.Struct.keyed("Money", [{"amount", Decimal.new("1")}, {"currency", Dextrin.Keyword.new("gbx")}])
  ...
  test "a plain string-keyed map is also accepted (no struct-literal wire form, but validated)", %{registry: registry} do
    assert {:ok, encoded} =
             Dextrin.encode(%{"amount" => Decimal.new("1"), "currency" => Dextrin.Keyword.new("usd")},
               registry: registry,
               schema: "Money"
             )
  ...
  test "encode_binary/2 supports the same schema: opt", %{registry: registry} do
    money = Dextrin.Struct.keyed("Money", [{"amount", Decimal.new("1")}, {"currency", Dextrin.Keyword.new("usd")}])
    ...
    assert {:error, %Dextrin.Error{}} = Dextrin.encode_binary(bad, registry: registry, schema: "Money")
  end
  ...
  describe "{:reference, name} checking at encode time (Dextrin.Registry.put_struct_module/3)" do
    test "a struct of the wrong module is rejected for a reference-typed field", %{registry: registry} do
      registry = Dextrin.Registry.put_struct_module(registry, "Address", TestAddress)

      wrong = %{"home" => struct(TestCompany, ein: "1")}
      assert {:error, %Dextrin.Error{message: message}} = Dextrin.encode(wrong, registry: registry, schema: "Person")
      assert message =~ "home"
    end
    ...
    test "a Dextrin.Struct checks its own name directly, with no put_struct_module needed", %{registry: registry} do
      right = Dextrin.Struct.keyed("Person", [{"home", Dextrin.Struct.keyed("Address", [{"street", "Main St"}])}])
      assert {:ok, _} = Dextrin.encode(right, registry: registry, schema: "Person")

      wrong = Dextrin.Struct.keyed("Person", [{"home", Dextrin.Struct.keyed("NotAddress", [{"street", "Main St"}])}])
      assert {:error, %Dextrin.Error{}} = Dextrin.encode(wrong, registry: registry, schema: "Person")
    end
  end
end
