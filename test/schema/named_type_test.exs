defmodule Dextrin.Schema.NamedTypeTest do
  @moduledoc """
  Named types: a `.dxns` entry that isn't a
  ...
  test "a field using a named type rejects a value violating its refinement", %{
    registry: registry
  } do
    assert {:error, _} =
             Dextrin.decode("%Money{amount: 0, discount: 12.5, tags: []}", registry: registry)

    assert {:error, _} =
             Dextrin.decode("%Money{amount: 5, discount: 150.0, tags: []}", registry: registry)
  end

  test "a named type used inside another type_expr form (list-of) is enforced element-wise", %{
    registry: registry
  } do
    assert {:error, _} =
    ...
    assert {:error, _} =
             Dextrin.decode(
               ~s(%Money{amount: 5, discount: 1.0, tags: ["#{String.duplicate("x", 21)}"]}),
               registry: registry
             )
  end

  test "named types are visible in Dextrin.Registry.type_aliases and resolvable directly", %{
    registry: registry
  } do
  ...
  test "a named type can't reference another named type declared in the same document (v1 boundary)" do
    ...
    assert {:ok, {:list_of, {:reference, "Base"}}} =
             Dextrin.Registry.fetch_type_alias(registry, "Derived")
  end

  test "put_type_alias/3 seeds a registry by hand, without going through compile/3" do
    registry =
      Dextrin.Registry.new()
      |> Dextrin.Registry.put_type_alias(
        "Even",
        {:refine, {:primitive, "integer"}, %{"multiple-of" => 2}}
      )
