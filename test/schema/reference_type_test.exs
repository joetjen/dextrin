defmodule Dextrin.Schema.ReferenceTypeTest do
  @moduledoc """
  `{:reference, name}` type-checking (DESIGN.md §10): a field typed as
  a struct reference must reject a value that's a *different*,
  otherwise-valid struct — even when that struct has its own
  registered schema and materializes successfully on its own terms.
  Closed via `Dextrin.Schema.Validated`, an internal provenance
  wrapper `Dextrin.Schema.Validator.materialize/4` produces and
  ...
  test "the correct struct is still accepted", %{registry: registry} do
    assert {:ok, %{"home" => %{"street" => "Main St"}, "homes" => []}} =
             Dextrin.decode(~s(%Person{home: %Address{street: "Main St"}, homes: []}), registry: registry)
  end

  test "an unregistered (opaque) struct is still rejected by its own name, as before", %{registry: registry} do
    assert {:error, %Dextrin.Error{}} =
             Dextrin.decode(~s(%Person{home: %SomeOtherThing{x: 1}, homes: []}), registry: registry)
  end

  test "the wrong struct nested inside a list-of reference is also rejected", %{registry: registry} do
    assert {:error, %Dextrin.Error{}} =
             Dextrin.decode(
  ...
  test "schema enforcement for references also applies to .dxnb" do
    ...
    wrong =
      %Dextrin.Struct{
        name: "Person",
        fields: {:positional, [%Dextrin.Struct{name: "Company", fields: {:positional, ["1"]}}, []]}
      }

    # validate: false -- deliberately building invalid wire bytes here
    # to exercise the *decode*-side check below; encode's own
    # automatic validation (DESIGN.md §10/§12) would otherwise catch
    # this exact mismatch before it ever became bytes.
  ...
  test "a materializer producing a real Elixir struct never leaks the internal provenance wrapper" do
    ...
    registry =
      ...
      |> Dextrin.Registry.put_struct_materializer("Address", fn %{street: s} -> {:ok, %NamedAddress{street: s}} end)

    {:ok, registry} = Dextrin.Schema.compile(doc, registry)

    assert {:ok, %NamedAddress{street: "A"}} = Dextrin.decode(~s(%Address{street: "A"}), registry: registry)
  ...
  test "a struct at the document root with no enclosing schema at all is still stripped cleanly", %{
    ...
    assert {:ok, %{"street" => "A"}} = Dextrin.decode(~s(%Address{street: "A"}), registry: registry)
  end

  test "encode-time reference checking now recurses fully into the referenced value's own schema", %{
    registry: registry
  } do
    right = Dextrin.Struct.keyed("Person", [{"home", Dextrin.Struct.keyed("Address", [{"street", "A"}])}, {"homes", []}])
    assert {:ok, _} = Dextrin.encode(right, registry: registry)

    wrong_name =
      Dextrin.Struct.keyed("Person", [{"home", Dextrin.Struct.keyed("Company", [{"ein", "1"}])}, {"homes", []}])
    ...
    # DESIGN.md §10); now it's caught, same as decode already would.
    internally_invalid =
      Dextrin.Struct.keyed("Person", [{"home", Dextrin.Struct.keyed("Address", [{"street", 123}])}, {"homes", []}])

    assert {:error, %Dextrin.Error{message: message}} = Dextrin.encode(internally_invalid, registry: registry)
